...
export function buildLandingPageScorecardPrompt(topic: string, body: string, callToAction: string): string {
...
Score against exactly these six criteria in the breakdown, plus overall persuasiveness. For each criterion: state whether it was met, and give a one-sentence, specific note (quote the offending text if it failed).
