...
*Extension — Executive Editor's Writing Guidelines + a real bug (2026-07-18):* The user's actual "Content Studio" writing-style document (distinct from the Brief/Blog/Case Study OS docs — a plain-English language/style guide) added two more real, portable criteria: an explicit, much larger banned-words list (~30 specific terms — "leverage," "synergy," "seamless," "scalable," "optimize," etc. — replacing the old vague "and similar overused marketing language" catch-all with actual named words), and a no-alliteration rule. Both verified live and grading correctly — the expanded list caught real usage ("optimized," "scalable") the old short list would have missed. Two items from the same doc deliberately not added: "unexpected language" and customer-psychology/emotion-over-feature framing — both are real principles but too subjective for the evaluator's binary met/not-met shape, and the doc's own examples for them are tagline/email-specific, a mode already deprioritized. Left as an open question rather than force-fit into a criterion.
