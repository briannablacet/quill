...
*Status — retrieval layer (2026-07-17):* Built and verified against real documents (`lib/agents/brand-profile.ts`), not test data — the user's actual Anomali Writing Style Guide and Message Platform docs (.docx). **Key design decision:** real style guides are unstructured prose, not key-value config, so ingestion uses `generateObject` extraction (same pattern as everything else in Quill) rather than a regex/keyword parser — verified accurate against the source (real terminology rules, the real tagline, real proof points with real numbers, nothing fabricated). One `BrandProfileDoc` per user (no multi-tenancy, per §6 decision #1), adapted into the existing `WritingStyle`/`BrandVoice`/`Messaging` shape (`toPromptInputs`) rather than reshaping style-guide.ts around one document type. Wired into `generateContent` (blog_post) and `scoreContent` (blog_post) — the evaluator now grades against real brand rules (avoid-phrases, core style points, Oxford comma), not just generic article structure.

Live test findings (topic: real Anomali security content, not the AI-agent test topic used everywhere else): first draft scored C/71 — genuinely informative, not a miscalibration. It caught real, verified compliance gaps: passive voice, third-person instead of "you," overlong sentences. One real bug caught along the way: the added brand-rule content made the prompt long enough that `maxOutputTokens: 4000` truncated the draft mid-sentence; raised to 6000. Auto-regeneration (Phase 4) fired as designed and genuinely fixed "you"-address, conciseness, and list-usage — verified fix, not just a re-roll — landing at B/81. Active voice specifically did not fully resolve even after the correction pass (the model still produced some passive constructions, though fewer). Not a bug to chase further right now — the one-pass regeneration cap is working as designed (real improvement, not perfection, no runaway loop), and this is genuine signal that "consistent active voice across 1000+ words" is a harder constraint for the model to fully satisfy than one-off formatting rules (Oxford comma, terminology, banned phrases all passed cleanly both times).
