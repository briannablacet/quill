...
*Status — UI (2026-07-18):* Built and verified live. `app/page.tsx` (previously the inherited `chief-of-staff-dashboard` job dashboard) replaced entirely — that dashboard, `components/cos/*`, `lib/actions.ts`, `lib/ats-checker.ts`, and every job-search API route (`jobs`, `matches`, `resume`, `import-job`, `bookmarklet-link`, `capture`) were deleted outright rather than left as dead weight, confirmed via grep that nothing Quill-relevant depended on any of it. New real endpoints: `POST /api/content/generate` (validates and enqueues per mode — never runs the agent inline, matching the production task-queue architecture), `GET /api/content` (history list), `GET /api/content/[contentId]` (polling target, also resolves `regeneratedTo` so the UI can follow a regeneration chain to its latest draft), `GET /api/tasks/[taskId]` (lets the UI learn a task's `result.contentId` once the worker finishes it). `lib/session.ts` resolves the current user: a fixed dev user outside production (better-auth needs a real `DATABASE_URL`, unset in local dev), real session in production — same shape as the existing dev-route bypass pattern, not a new convention.

The client polls rather than drives the queue inline: on generate, it enqueues, then polls `/api/tasks/[taskId]` for the `contentId`, then polls `/api/content/[contentId]` until scored, nudging `/api/worker` each tick (harmless no-op in production if `CRON_SECRET` blocks it — cron ticks independently regardless). UI covers 5 of 6 modes (`blog_post`, `landing_page`, `case_study`, `social_media`, `battlecard`) — `taglines` stays backend-only per the earlier UI-scoping decision.

Two real bugs caught during live verification, not hypothetical: (1) the initial poll ceiling (80 polls, ~2 min) cut off before a full generate→score→regenerate→score chain finished — observed live taking 3+ minutes end to end; raised to 280 polls (~7 min). (2) Switching mode tabs while a previous generation was still polling didn't stop that polling interval, so the old mode's result kept overwriting the new tab's (reset) state on every subsequent tick — fixed by stopping the interval in the mode-switch handler before resetting state. Verified live end-to-end: a real `blog_post` draft scored 65/C, triggered a real regeneration, scored 68/C, and the UI correctly rendered the grade badge, colored score bar, "Improved from 65," full criteria breakdown (including the em-dash/banned-words/choppy-sentence criteria from earlier in Phase 5), fixGuidance, and styleNotes — all real data, no mocks.

*Extension — re-theme + navigation + the 3 missing pages (2026-07-18):* First live use surfaced two real gaps. (1) The look didn't match the approved canvas mockup — fetched that artifact's actual CSS (not reinterpreted from memory) and remapped Quill's theme tokens to it exactly: warm cream "paper" background, muted amber accent, soft-tint success/warn/destructive colors, serif for content (system font stack, no external font loading) with sans for UI chrome — same split the mockup uses. Since shadcn's components already consume these CSS variables, the whole app picked up the new look from one file change; only a `GradePill` (circular soft-tint grade badge, shared between the Scorecard view and History list) needed a real component change. (2) There was no navigation at all — competitive intel, SERP monitoring, and ideation existed only as backend agents verified through dev routes, with no way to reach them from the UI. Added a persistent nav bar and a real page for each: Competitive Intel (keyword or named-competitor input), SERP Monitor (keyword input, shows the diff against the last snapshot), Ideas (one button). All three follow the same enqueue-then-poll pattern as content generation — no new execution path. Verified live: all three correctly displayed real historical data from earlier backend testing, and a fresh Ideas generation ran end-to-end citing current Scorecard scores.
...
*Status — retrieval layer (2026-07-17):* Built and verified against real documents (`lib/agents/brand-profile.ts`), not test data — a real Anomali Writing Style Guide and Message Platform doc (.docx) the user had on hand. **Note on provenance:** this is not the user's employer (ServiceNow) — it was a sample doc used purely to stress-test ingestion against real, messy, unstructured documents rather than fabricated test data. The framework is brand-agnostic (nothing in the code references Anomali specifically); swapping in real ServiceNow docs later is a re-ingestion call, not a rebuild. Single profile per user for now — no need for multiple named profiles yet, revisit if/when ServiceNow work needs Moveworks + ServiceNow messaging to coexist.

**Key design decision:** real style guides are unstructured prose, not key-value config, so ingestion uses `generateObject` extraction (same pattern as everything else in Quill) rather than a regex/keyword parser — verified accurate against the source (real terminology rules, the real tagline, real proof points with real numbers, nothing fabricated). One `BrandProfileDoc` per user (no multi-tenancy, per §6 decision #1), adapted into the existing `WritingStyle`/`BrandVoice`/`Messaging` shape (`toPromptInputs`) rather than reshaping style-guide.ts around one document type. Wired into all five scorable/generatable modes except `taglines` (deliberately excluded — not used for real brand work per user direction). `scoreContent` now checks real brand rules for `blog_post`, `landing_page`, and `case_study`.

Live test findings (topic: real security-industry content, not the AI-agent test topic used everywhere else): first blog_post draft scored C/71 — genuinely informative, not a miscalibration. It caught real, verified compliance gaps: passive voice, third-person instead of "you," overlong sentences. One real bug caught along the way: the added brand-rule content made the prompt long enough that `maxOutputTokens: 4000` truncated the draft mid-sentence; raised to 6000 (blog_post) and similarly upsized for social_media's per-platform budgets. Auto-regeneration (Phase 4) fired as designed and genuinely fixed "you"-address, conciseness, and list-usage — verified fix, not just a re-roll — landing at B/81. Active voice specifically did not fully resolve even after the correction pass. Not a bug to chase further right now — the one-pass regeneration cap is working as designed, and this is genuine signal that "consistent active voice across 1000+ words" is a harder constraint for the model than one-off formatting rules (Oxford comma, terminology, banned phrases all passed cleanly both times).

Two more real things found extending this to the other modes:
1. **Schema bug:** `social_media`'s schema was the only top-level array (`z.array(z.string())`) in the whole codebase — every other `generateObject` call uses a top-level object. It started intermittently failing ("could not parse the response") once brand-rule content lengthened the prompt. Fixed by wrapping in an object (`{ posts: string[] }`), matching the pattern used everywhere else.
2. **Message-bleed (unresolved, worth watching):** in one social_media test, the model wove Anomali's actual product claims ("Anomali Copilot," "agentless," "Data Lake") into a post that was supposed to be about Quill, conflating the two as the same product. `messaging.keyMessages` gets injected as "weave in where relevant" regardless of topical fit — fine when the content is genuinely about the brand's own product, confusing when it isn't (a testing artifact here, since the test topic was about Quill while the stored profile is Anomali's, a mismatch that won't occur in real on-brand use — but the underlying lack of a relevance check is real). `battlecard` did not show this issue, likely because its "Our Advantages" section is tightly scoped to only the user-provided positioning text. Worth a relevance guard if this shows up in real use.

*Extension — regeneration for landing_page and case_study (2026-07-17):* Auto-regeneration (Phase 4) previously only knew how to rebuild a `blog_post` payload. Extended to `landing_page` and `case_study`: the writer now stores the full original structured payload (`meta.originalPayload`) at generation time, and the orchestrator reconstructs it on a low score, folding fix guidance into the `additionalDetails` field both prompts already support, rather than replacing the structured fields (title/CTA/bullets, or customer/problem/solution/results). One real bug caught while wiring this up: `regeneratedFrom` was captured from the payload for `blog_post` only — `landing_page`/`case_study` never persisted it on the `ContentDoc`, so the orchestrator's one-pass cap (`if (content.regeneratedFrom) return`) could never actually engage for those two modes, risking an unbounded regeneration chain on a stubborn low scorer. Fixed in `writer.ts` to capture and store it for all three modes. Verified live: `landing_page` went 78 → 80 (B → B, capped after one pass, no third attempt); `case_study` went 86 → 90 (B → A, capped after one pass) with every source fact — customer, role, company, problem/solution/results — byte-identical across both drafts, confirming regeneration never touches the facts, only the prose.

*Extension — real prose-quality criteria from the user's production editorial systems (2026-07-18):* The user shared real, in-use editorial documents from their work editorial system (Moveworks Brief OS, Blog OS, Case Study System, and a Content Studio Feedback Format Guide) — the same category of "real prompts over invented ones" upgrade that the Skribil extraction and brand-profile ingestion already went through. Three portable (brand-agnostic) prose-quality rules from these were folded into `blog_post`'s Scorecard criteria (`content-scorecard.ts`): em dash cadence (max one per paragraph, never consecutive), no choppy sentence sequences (short, similarly-structured sentences that read like unbulleted bullet points — a real, named model habit, not a hypothetical), and no duplicated phrasing across sections. Deliberately not ported: the format guide's weighted five-category scorecard (Legal/Messaging/Prose/Brand/Claim-grounding with point allocations), structural-location flag anchoring (H2/bullet-level), silent mode, and exact-replacement-text fixes — the user confirmed Quill's existing grade/score/breakdown/fixGuidance shape is sufficient and explicitly said not to rebuild around the richer format. Brand-and-employer-specific rules from these same documents (Moveworks positioning statements, Reasoning Engine/Search+Action/Agent Studio framing, statistic-source whitelist, specific prohibited terms) belong in the brand profile layer (`brand-profile.ts`), not the core Scorecard — no new schema needed there, it already has the right shape (avoidPhrases, customRules, keyMessages), just needs real ingestion whenever a real ServiceNow/Moveworks profile is ready.

Live test of the new criteria surfaced a genuine, unplanned finding: **regeneration is not guaranteed to improve the score.** A rewrite that fixes every explicitly-flagged issue can introduce *new* violations of criteria that were previously passing — observed twice independently (73→69, then again 71→68), both times because the fix pass dropped direct "you" address or introduced a new duplicated phrase while fixing what was flagged. The orchestrator's fixGuidance only tells the rewrite what was wrong before, not what to protect. **Fix applied (safety net, not a root-cause fix):** `enqueueFollowUps` now compares a regenerated draft's score against the draft it was regenerated from and records `regenerationOutcome: "improved" | "regressed" | "unchanged"` plus `previousScore` on the `ContentDoc` — so a worse rewrite is visible and queryable rather than silently standing as "the" result. Verified live: caught the second regression (71→68) correctly and automatically. The root cause (rewrite prompts don't protect previously-passing criteria) is intentionally not fixed yet — flagged as a real next step if it keeps happening, same treatment as the message-bleed finding above.

*Extension — Executive Editor's Writing Guidelines + a real bug (2026-07-18):* The user's actual "Content Studio" writing-style document (distinct from the Brief/Blog/Case Study OS docs — a plain-English language/style guide) added two more real, portable criteria: an explicit, much larger banned-words list (~30 specific terms — "leverage," "synergy," "seamless," "scalable," "optimize," etc. — replacing the old vague "and similar overused marketing language" catch-all with actual named words), and a no-alliteration rule. Both verified live and grading correctly — the expanded list caught real usage ("optimized," "scalable") the old short list would have missed. Two items from the same doc — "unexpected language" and customer-psychology/emotion-over-feature framing — were too subjective for the evaluator's binary met/not-met `breakdown` shape, so they got their own lane instead: a new optional `styleNotes: string[]` field on the Scorecard schema (`content-scorecard.ts`), explicitly instructed to never factor into score, grade, or breakdown — pure qualitative craft commentary, 0-3 notes, empty array if nothing stands out. Verified live: real, specific observations came back ("the 2:14 a.m.-alert cold open is a very common SOC-writing device," "stays mostly in feature/architecture language... without attention to felt relief"), cleanly separated from the 25-item graded breakdown in both drafts. Since the schema is shared across all three scorable modes, `landing_page`/`case_study` can technically populate it too, but only `blog_post`'s prompt currently asks for it.

**Real bug found and fixed along the way:** during this testing, one regenerated draft came back with a completely empty body — `generateText` returned nothing, and nothing anywhere in `writer.ts` checked for that, so it would have silently persisted as a normal "done" `ContentDoc` (and then correctly but unhelpfully scored 0/F later, with the evaluator noting "no text exists to check"). Every other mode already validates its *inputs* (case_study requires facts, battlecard requires enough fetched page text) but nothing validated generation *output*. Fixed: a single check before the DB insert in `generateContent` now throws if `body`/`items` comes back empty, which routes through the existing retry-with-backoff logic in `lib/tasks.ts` instead of silently completing. Note: the empty-response condition itself is nondeterministic and did not reproduce on a follow-up test run, so the guard's logic was verified by inspection and passed typecheck, but the actual retry-on-failure path was not independently re-triggered.
